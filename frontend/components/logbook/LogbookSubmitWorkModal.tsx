...
export default function LogbookSubmitWorkModal({
  ...
  const [fileUrl, setFileUrl] = useState<string>(existingSubmission?.file_url || '');
  const [fileName, setFileName] = useState<string>(existingSubmission?.file_name || '');
  const [fileSize, setFileSize] = useState<string>('');
  const [explanationText, setExplanationText] = useState<string>(existingSubmission?.explanation_text || '');
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            {fileUrl ? (
              ...
              <div
                onClick={() => fileInputRef.current?.click()}
                className="border-2 border-dashed border-[#E7EAF3] dark:border-slate-800 hover:border-[#5B4BFF] dark:hover:border-[#5B4BFF] rounded-2xl p-6 text-center transition-all cursor-pointer bg-[#F6F8FC]/50 dark:bg-slate-850/50 hover:bg-[#EEECFF]/30 space-y-2 group"
              >
