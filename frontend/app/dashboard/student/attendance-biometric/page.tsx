...
export default function StudentBiometricAttendancePage() {
  const [logs] = useState([
    ...
    { id: 2, device: 'BIO-[#5B4BFF]-Main', type: 'Fingerprint Biometric', student: 'Rahul Verma (2024106259)', time: '01:05 PM', status: 'VERIFIED', confidence: '100%' },
  ]);
  ...
  return (
    <div className="flex min-h-screen bg-[#F6F8FC] dark:bg-[#0F172A] text-[#1B1E28] dark:text-slate-100 font-sans transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Header title="Student View — My Bio-Metric & CCTV Scan Log" />
          ...
          <div className="p-6 rounded-[22px] bg-white dark:bg-slate-900 border border-[#E7EAF3] dark:border-slate-800 shadow-soft flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-base font-black text-[#1B1E28] dark:text-white uppercase tracking-wider flex items-center gap-2">
                <span>📹</span>
                <span>Personal Bio-Metric & CCTV Recognition Logs</span>
              </h2>
              <p className="text-xs text-[#7B8794] font-medium mt-1">
                Real-time campus turnstile and CCTV AI facial check-in logs for your profile.
              </p>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-[#F6F8FC] dark:bg-slate-800/80 text-[#7B8794] uppercase tracking-wider border-b border-[#E7EAF3] dark:border-slate-700">
                    <th className="p-3 font-extrabold">Device & Location</th>
                    <th className="p-3 font-extrabold">Scanner Type</th>
                    <th className="p-3 font-extrabold">Student Name</th>
                    <th className="p-3 font-extrabold text-center">Scan Time</th>
                  ...
                  {logs.map((log) => (
                    <tr key={log.id} className="hover:bg-[#F6F8FC]/50 dark:hover:bg-slate-800/40 transition">
                      ...
                      <td className="p-3 text-right">
                        <span className="px-2.5 py-1 rounded-xl text-[10px] font-black uppercase bg-emerald-500/10 text-emerald-600 border border-emerald-500/30">
                          {log.status}
                        </span>
                      </td>
                    </tr>
