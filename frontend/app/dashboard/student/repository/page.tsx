...
export default function StudentRepositoryPage() {
  ...
  const fetchRepositories = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:8081/api/v1/repository/list?student_reg_no=${regNo}&tenant=${slug}`, {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRepo) {
        ...
        const res = await fetch(`http://localhost:8081/api/v1/repository/${editingRepo.repo_id}`, {
          method: 'PUT',
          headers: {
        ...
        const res = await fetch(`http://localhost:8081/api/v1/repository/submit`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
