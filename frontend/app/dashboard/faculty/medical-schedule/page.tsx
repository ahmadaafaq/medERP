...
export default function FacultyMedicalSchedulePage() {
  ...
  const fetchMySchedule = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/v1/medical-timetable/schedule/my?tenant=${tenantSlug}`, {
    ...
    <div className="min-h-screen bg-[#F6F8FC] dark:bg-slate-950 p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-200/80 dark:border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-emerald-600 to-teal-600 flex items-center justify-center text-white shadow-lg shadow-emerald-500/25 shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full text-xs font-extrabold bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                Faculty Medical Portal
              </span>
              <span className="text-xs font-semibold text-slate-400">
                Assigned Clinical & Academic Roster
              </span>
            </div>
                      ...
                      >
                        <div className="flex items-center justify-between gap-1">
                          <span className="text-[11px] font-extrabold uppercase tracking-wider text-indigo-600 dark:text-indigo-400">
                            {entry.department_name || 'Department'}
                          </span>
                        ...
                        <div className="grid grid-cols-2 gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span className="truncate">{entry.professional_year_name || '1st Prof'}</span>
                          </div>
                        </div>
