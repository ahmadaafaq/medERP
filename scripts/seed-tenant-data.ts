...
async function seedData() {
  ...
  try {
    ...
    console.log('🌱 Connected to PostgreSQL. Seeding data for tenant_srms...');

    // Ensure public.tenants has 'srms'
    await client.query(`
      INSERT INTO public.tenants (name, slug, domain, plan, primary_color, schema_provisioned)
      VALUES ('SRMS Medical College', 'srms', 'srms.mederp.app', 'enterprise', '#6366F1', true)
      ON CONFLICT (slug) DO UPDATE SET schema_provisioned = true;
    `);

    // Create schema tenant_srms
    await client.query(`CREATE SCHEMA IF NOT EXISTS "tenant_srms"`);
    await client.query(`SET search_path TO "tenant_srms"`);
    ...
    console.log('✅ Seed completed successfully for tenant_srms!');
  } catch (err) {
