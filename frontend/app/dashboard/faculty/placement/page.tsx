  ...
  const handleUpdateStatus = async (appId: number, status: string) => {
    try {
      ...
      await axios.patch(`/api/placement-drive/shortlist?tenant=${tenant}`, { application_id: appId, status }).catch(async () => {
        return axios.patch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8081/api/v1'}/placement-drive/shortlist?tenant=${tenant}`, { application_id: appId, status });
      });
  ...
  const handleExportCompany = async (driveId: number, companyName: string, status?: string) => {
    try {
      ...
      const res = await axios.get(url).catch(async () => {
        return axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8081/api/v1'}/placement-drive/export?tenant=${tenant}&drive_id=${driveId}${status ? `&status=${status}` : ''}`);
      });
