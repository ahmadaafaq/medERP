...
async function seedData() {
  ...
  try {
    ...
    const anatDeptId = deptRes.rows.find((r) => r.code === 'ANAT')?.id;
    const physDeptId = deptRes.rows.find((r) => r.code === 'PHYS')?.id;
    ...
    const anatSubjId = subjRes.rows.find((r) => r.code === 'ANAT101')?.id;
    const physSubjId = subjRes.rows.find((r) => r.code === 'PHYS101')?.id;
    ...
    await client.query(`
      DO $$ BEGIN
        IF NOT EXISTS (
          ...
          WHERE conrelid = 'topics'::regclass AND conname = 'topics_code_unique'
        ) THEN
    ...
    for (const t of topicsData) {
      await client.query(
        `INSERT INTO topics (subject_id, code, name) VALUES ($1, $2, $3)
         ON CONFLICT ON CONSTRAINT topics_code_unique DO UPDATE SET name = EXCLUDED.name;`,
        [t.subjId, t.code, t.name]
      );
    }

    // 5. Competencies Master
    await client.query(`
    ...
    await client.query(`
      DO $$ BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_constraint
          WHERE conrelid = 'competencies'::regclass AND conname = 'competencies_code_unique'
        ) THEN
          ALTER TABLE competencies ADD CONSTRAINT competencies_code_unique UNIQUE (code);
        END IF;
      END $$;
    `);
    ...
    for (const c of competenciesData) {
      await client.query(
        `INSERT INTO competencies (code, description) VALUES ($1, $2)
         ON CONFLICT ON CONSTRAINT competencies_code_unique DO UPDATE SET description = EXCLUDED.description;`,
        [c.code, c.desc]
      );
    }

    // 6. 20 CBME MCQs for Professional 1 (Anatomy & Physiology) in Question Bank
    await client.query(`
    ...
    // Clear previously seeded Professional 1 MCQs to avoid duplicates on re-run
    await client.query(`
      DELETE FROM question_bank
      WHERE professional_phase = 'Phase 1 (1st Professional MBBS)'
        AND competency_code = ANY($1::text[]);
    `, [mcqsData.map((m) => m.code)]);
