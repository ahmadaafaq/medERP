  ...
  const handleUpdateStatus = async (appId: number | string, status: string) => {
    try {
      ...
      setApplicantsList((prev) =>
        prev.map((a) => (String(a.application_id) === String(appId) ? { ...a, status } : a))
      );
      ...
      alert(e?.response?.data?.message || e?.message || 'Failed to update applicant status');
    } finally {
                    ...
                    {applicantsList.map((app) => (
                      <tr key={app.application_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="p-3.5 text-right space-x-1.5">
                          ...
                          >
                            {String(updatingAppId) === String(app.application_id) ? '...' : 'Place'}
                          </button>
