...
async function main() {
  ...
  try {
    ...
    console.log('topicSubmissions query success! Result length:', topicSubmissions?.length);
    console.log('topicSubmissions list:', topicSubmissions);
  } catch (err) {
    console.error('ERROR inside tenantSchemaService.queryInTenant:', err);
  }
  await app.close();
}
main();
