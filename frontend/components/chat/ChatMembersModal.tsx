  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#FFFFFF] dark:bg-slate-900 border border-[#E7EAF3] dark:border-slate-800 rounded-[22px] shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 bg-[#F6F8FC] dark:bg-slate-900/80 border-b border-[#E7EAF3] dark:border-slate-800 space-y-3">
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-[#4E5969] dark:text-slate-400" />
            <input
              type="text"
              placeholder="Search members by name..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2 text-xs rounded-xl border border-[#E7EAF3] dark:border-slate-700 bg-white dark:bg-slate-800 text-[#1B1E28] dark:text-white placeholder-[#4E5969] focus:outline-hidden focus:ring-2 focus:ring-[#5B4BFF]"
            />
          </div>
        ...
        <div className="p-4 space-y-2 overflow-y-auto flex-1 max-h-[400px]">
          {loading ? (
            ...
            filteredMembers.map((member) => {
              const isFac = member.role === 'FACULTY';
                    ...
                    {member.avatar_url ? (
                      <img
                        ...
                        onError={(e) => {
                          ...
                          const fb = (e.target as HTMLElement).parentElement?.querySelector('.member-fallback') as HTMLElement;
                          if (fb) fb.style.display = 'flex';
                        }}
                      />
                    ) : null}
                    <div
                      className={`w-10 h-10 rounded-xl font-black text-xs items-center justify-center uppercase shrink-0 shadow-xs ring-2 member-fallback ${
