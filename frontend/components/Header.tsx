...
export default function Header({ title = 'MedERP Portal' }: HeaderProps = {}) {
  ...
  const fetchChatUnread = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8081/api/v1'}/chat/unread-count?tenant=${tenantSlug}`, {
        headers: {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8081/api/v1'}/auth/change-password`, {
        method: 'POST',
        headers: {
