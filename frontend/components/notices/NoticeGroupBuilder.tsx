...
export default function NoticeGroupBuilder({ rules, onChange }: NoticeGroupBuilderProps) {
  ...
  const loadDynamicOptions = async () => {
    ...
    try {
      ...
      const btRes = await fetch(`http://localhost:3001/api/v1/college-master/batches?tenant=${slug}`, {
      ...
      // 3. Fetch departments
      const deptRes = await fetch(`http://localhost:3001/api/v1/college-master/departments?tenant=${slug}`, {
        headers: { Authorization: `Bearer ${token}` },
      }).catch(() => null);
