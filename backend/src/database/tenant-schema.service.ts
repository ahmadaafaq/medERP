...
export class TenantSchemaService implements OnApplicationBootstrap {
  ...
  async ensureLatestSchema(slug: string): Promise<void> {
    ...
    try {
      ...
      // ── Notices & Circulars Module Tables ─────────────────────────────────
      await runner.query(`
        ...
        CREATE TABLE IF NOT EXISTS "${schema}".notice_attachments (
          id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
          notice_id UUID NOT NULL REFERENCES "${schema}".notices(id) ON DELETE CASCADE,
  ...
  private async seedDefaultData(runner: QueryRunner, slug: string): Promise<void> {
    ...
    // 10. Seed authentic Notices & Circulars with target rules, attachments, and recipient fan-outs
    try {
      const noticeCountRes = await runner.query(`SELECT COUNT(*) as count FROM "${schema}".notices`);
      if (parseInt(noticeCountRes[0]?.count || '0', 10) === 0) {
        const adminUserRes = await runner.query(`SELECT id FROM "${schema}".users WHERE role IN ('COLLEGE_ADMIN', 'SUPER_ADMIN') LIMIT 1`);
        const adminUserId = adminUserRes[0]?.id || null;

        const allUsers = await runner.query(`SELECT id, role FROM "${schema}".users`);
        ...
        if (n1) {
          await runner.query(`
            ...
            VALUES ($1, 'all', 'all', 'Everyone in College');
          `, [n1]);
          await runner.query(`
            INSERT INTO "${schema}".notice_attachments (notice_id, file_name, file_type, file_url, file_size_kb)
            VALUES ($1, 'IT_Cell_Portal_Notification_Guide.pdf', 'pdf', '/uploads/notices/sample_guide.pdf', 420);
          `, [n1]);
        ...
        if (n2) {
          await runner.query(`
            INSERT INTO "${schema}".notice_targets (notice_id, target_type, target_value, target_label)
            VALUES ($1, 'role', 'STUDENT', 'All Students');
          `, [n2]);
        ...
        if (n3) {
          await runner.query(`
            INSERT INTO "${schema}".notice_targets (notice_id, target_type, target_value, target_label)
            VALUES ($1, 'role', 'FACULTY', 'All Faculty');
          `, [n3]);
        ...
        if (n4) {
          await runner.query(`
            INSERT INTO "${schema}".notice_targets (notice_id, target_type, target_value, target_label)
            VALUES ($1, 'batch_year', '2023-MBBS', '2023 MBBS Batch');
          `, [n4]);
          await runner.query(`
            INSERT INTO "${schema}".notice_attachments (notice_id, file_name, file_type, file_url, file_size_kb)
            VALUES ($1, 'Physiology_Logbook_Competency_Checklist.xlsx', 'xlsx', '/uploads/notices/sample_checklist.xlsx', 185);
          `, [n4]);
          for (const u of studentUsers) {
            await runner.query(`
              INSERT INTO "${schema}".notice_recipients (notice_id, user_id, is_read)
        ...
        // Notice 5: Internal Exam Datesheet
        const n5 = (await runner.query(`
          INSERT INTO "${schema}".notices (title, body, priority, category, creator_name, creator_role, created_by, status, created_at)
          VALUES ('[Super Admin - admin] Internal Assessment Schedule', 'Phase I and Phase II MBBS Internal Assessment Theory & Practical schedules are officially notified. Download the attached official circular and seating plan.', 'important', 'announcement', 'Examination Controller', 'Super Admin', $1, 'sent', NOW() - INTERVAL '3 days')
          RETURNING id;
        `, [adminUserId]))[0]?.id;
        ...
        if (n5) {
          ...
          await runner.query(`
            INSERT INTO "${schema}".notice_attachments (notice_id, file_name, file_type, file_url, file_size_kb)
            VALUES 
              ($1, 'Internal_Assessment_Datesheet_2026.pdf', 'pdf', '/uploads/notices/sample_datesheet.pdf', 650),
              ($1, 'Seating_Arrangement_Plan.docx', 'docx', '/uploads/notices/sample_plan.docx', 310);
          `, [n5]);
          for (const u of allUsers) {
            await runner.query(`
              INSERT INTO "${schema}".notice_recipients (notice_id, user_id, is_read)
        ...
        // Seed 2 Default Notice Group Templates
        await runner.query(`
          INSERT INTO "${schema}".notice_group_templates (name, description, target_rules, created_by)
          VALUES 
            ('All MBBS 2023 Batch Students', 'Standard group targeting all active 2023 MBBS batch undergraduates', '[{"target_type": "batch_year", "target_value": "2023-MBBS", "target_label": "2023 MBBS Batch"}]'::jsonb, $1),
            ('All Teaching Faculty & HODs', 'Target group containing all academic department faculty members', '[{"target_type": "role", "target_value": "FACULTY", "target_label": "All Faculty"}]'::jsonb, $1)
          ON CONFLICT DO NOTHING;
        `, [adminUserId]);
      }
    } catch (e) {
      this.logger.error('Error seeding default notices:', e);
    }
