...
async function findStudentFKs() {
  ...
  try {
    ...
    `);
    console.log('📌 Foreign Key tables referencing students:', res.rows.map(r => `${r.table_name}.${r.column_name}`));
  } catch (err) {
    console.error('Error finding FKs:', err);
  } finally {
    await client.end();
  }
}
